	...
	if thing:isItem() then
		...
		if thing:isCreature() then
			if thing:isPlayer() then
				description = string.format("%s\nClient Version: %s.", description, thing:getClient().version)
			end
		end
...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
	...
	if self:getStorageValue(warStorages.ssaMight) == 2 and toPosition.x == CONTAINER_POSITION and ((toPosition.y == CONST_SLOT_NECKLACE and item:getId() == ITEM_STONE_SKIN_AMULET) or (toPosition.y == CONST_SLOT_RING and item:getId() == MIGHT_RING)) then
		self:sendCancelMessage("You can't equip this item in the city war.")
	return false
	end
...
function Player:onMoveCreature(creature, fromPosition, toPosition)
	if creature:isPlayer() and (creature:getTile():hasFlag(TILESTATE_PROTECTIONZONE) or creature:getStorageValue(Storage.isTraining) == 1) and self:getGroup():hasFlag(PlayerFlag_CanPushAllCreatures) == false then
		self:sendCancelMessage(RETURNVALUE_NOTPOSSIBLE)
	return false
	end
	return true
