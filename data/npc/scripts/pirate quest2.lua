...
local function creatureSayCallback(cid, type, msg)
	...
	if state == -1 then
	npcHandler:say({"Por supuesto, necesitaremos de tu ayuda por aqui...", "Podrias partir por matar a los 3 bosses...", "Luego de eso podre entregarte misiones mas relevantes"}, cid)
	...
	return true
	elseif state == 1 then
	if player:getStorageValue(Storage.PiratsTails.RatMiral.BossCounter) < 3 or player:getStorageValue(Storage.PiratsTails.RatMiral.LastKilled) ~= 1 then
	...
	player:setStorageValue(Storage.PiratsTails.RatMiral.ClickCounter, 1)
		player:addMount(175)
	...
	elseif state == 2 then
	if player:getStorageValue(Storage.PiratsTails.RatMiral.FairyFreeCount) < 5 or player:getStorageValue(Storage.ThreatenedDreams.RewardFinal1) == -1 then
