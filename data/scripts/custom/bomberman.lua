...
function bombEnter.onUse(player, item, position, fromPosition)
    ...
    if #playersWaiting < config.minPlayerLimit then
        player:sendTextMessage(MESSAGE_INFO_DESCR, 'Atleast ' .. config.minPlayerLimit .. ' players need to join.')
        return true
    end
    ...
    for i, position in ipairs(config.startPositions) do
	    ...
	    player:sendTextMessage(MESSAGE_INFO_DESCR, 'Use the command !bomb')
...
local function checkWinner()
    ...
    if lastPlayer and count == 1 then
        lastPlayer:sendTextMessage(MESSAGE_INFO_DESCR, 'You have won!')
        ...
        for i = 0, bombRadius do
            ...
            if topCreature and topCreature:isPlayer() then
                ...
                for _, player in pairs(BOMBERMAN_PLAYERS) do
                    if player then
                        player:sendTextMessage(MESSAGE_INFO_DESCR, deadMsg)
                    end
                end
...
function ta.onSay(player, words, param)
    ...
    if taExhaustion > os.time() then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'Slow down.')
        return false
    end

    if player:getStorageValue(config.storageInGame) <= 0 then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You may not place bombs outside of bomberman game.')
        return false
    end
    ...
    if playerBombsLeft <= 0 then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You have no bombs left!')
        return false
    end
