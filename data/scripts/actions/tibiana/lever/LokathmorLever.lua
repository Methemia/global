local Lokathmor = Action()
local config = {
	bossName = "Lokathmor",
	lockStorage = 50803017, -- globalstorage
	bossPos = Position(32751, 32689, 10),
	centerRoom = Position(32751, 32689, 10), -- Center Room  
	exitPosition = Position(32718, 32747, 10), -- Exit Position
	newPos = Position(32751, 32696, 10),
	range = 20,
...
function Lokathmor.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 32585 and item.actionid == 40095 then
		if player:getPosition() ~= Position(32721, 32749, 10) then
			return true
		end
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 32749, 10)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.LokathmorTimer) > os.time() then
				playerTile:sendTextMessage(MESSAGE_STATUS_SMALL, "You or a member in your team have to wait 20 hours to challange Lokathmor again!")
	...
	for i = 1, #specs do
		...
		if spec:isPlayer() then
			player:sendTextMessage(MESSAGE_STATUS_SMALL, "There's someone fighting with Lokathmor.")
			item:transform(32585)
			return true
		end
	end	
