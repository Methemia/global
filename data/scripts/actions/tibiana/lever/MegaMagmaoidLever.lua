local MegaMagmaoid = Action()
local config = {
	bossName = "The Mega Magmaoid",
	lockStorage = 55955883, -- globalstorage
	bossPos = Position(32501, 31161, 15),
	centerRoom = Position(32501, 31161, 15), -- Center Room  
	exitPosition = Position(32534, 31157, 15), -- Exit Position
	newPos = Position(32500, 31168, 15),
	range = 20,
...
function MegaMagmaoid.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 48712 then
		if player:getPosition() ~= Position(32530, 31159, 15) then
			return true
		end
-- Poss de los players			
	for x = 32530, 32534 do
	local playerTile = Tile(Position(x, 31159, 15)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.TheMegaMagmaoid) > os.time() then
...
-- poss de los players
	for x = 32530, 32534 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.TheMegaMagmaoid, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
...
MegaMagmaoid:aid(48712)
MegaMagmaoid:register()