local Ghulosh = Action()
local config = {
	bossName = "Ghulosh",
	lockStorage = 50803020, -- globalstorage
	bossPos = Position(32756, 32722, 10),
	centerRoom = Position(32756, 32722, 10), -- Center Room  
	exitPosition = Position(32744, 32771, 10), -- Exit Position
	newPos = Position(32756, 32729, 10),
	range = 20,
...
function Ghulosh.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 32585 and item.actionid == 40098 then
		if player:getPosition() ~= Position(32747, 32773, 10) then
			return true
		end
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 32773, 10)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GhuloshTimer) > os.time() then
	...
	for i = 1, #specs do
		...
		if spec:isPlayer() then
			player:sendTextMessage(MESSAGE_STATUS_SMALL, "There's someone fighting with Ghulosh.")
			item:transform(32585)
			return true
		end
	end	
...
-- poss de los players
	for x = 32747, 32751 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GhuloshTimer, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
