...
local Urmahlullu = Action()
local config = {
	bossName = "Urmahlullu the Immaculate",
	lockStorage = 50813020, -- globalstorage
	bossPos = Position(33919, 31648, 8),
	centerRoom = Position(33919, 31648, 8), -- Center Room  
	exitPosition = Position(33920, 31621, 8), -- Exit Position
	newPos = Position(33914, 31640, 8),
	range = 20,
...
function Urmahlullu.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 9545 then
...
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 31626, 8)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Kilmaresh.UrmahlulluTimer) > os.time() then
				playerTile:sendTextMessage(MESSAGE_STATUS_SMALL, "You or a member in your team have to wait 20 hours to challange Urmahlullu again!")
				item:transform(9826)
				return true
			end
		end
	...
	for i = 1, #specs do
		...
		if spec:isPlayer() then
			player:sendTextMessage(MESSAGE_STATUS_SMALL, "There's someone fighting with Urmahlullu.")
			item:transform(9826)
			return true
		end
	end	
...
-- poss de los players
	for x = 33918, 33922 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Kilmaresh.UrmahlulluTimer, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
