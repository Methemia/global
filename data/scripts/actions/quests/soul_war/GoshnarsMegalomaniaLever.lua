local Megalomania = Action()
local config = {
	bossName = "Goshnar's Megalomania",
	lockStorage = 50911112, -- globalstorage
	bossPos = Position(34093, 31027, 11),
	centerRoom = Position(34094, 31032, 11), -- Center Room  
	exitPosition = Position(34060, 31027, 11), -- Exit Position
	newPos = Position(34093, 31037, 11),
	range = 20,
...
function Megalomania.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38449 then
		if player:getPosition() ~= Position(34059, 31030, 11) then
			return true
		end
-- Poss de los players			
	for x = 34059, 34063 do
	local playerTile = Tile(Position(x, 31030, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GoshnarsMegalomania) > os.time() then
...
-- poss de los players
	for x = 34059, 34063 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GoshnarsMegalomania, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
...
Megalomania:aid(38449)
Megalomania:register()