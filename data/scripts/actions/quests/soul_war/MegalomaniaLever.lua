...
local config = {
	bossName = "Demon",
	lockStorage = 50815112, -- globalstorage
...
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 31030, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.Megalomania) > os.time() then
...
-- poss de los players
	for x = 34059, 34063 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.Megalomania, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
