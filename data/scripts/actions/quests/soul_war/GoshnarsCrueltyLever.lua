...
local config = {
	...
	lockStorage = 55811138, -- globalstorage
	bossPos = Position(33856, 31865, 7),
	centerRoom = Position(33856, 31865, 7), -- Center Room  
	exitPosition = Position(33880, 31872, 4), -- Exit Position
	newPos = Position(33858, 31873, 7),
	range = 20,
...
function Cruelty.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38457 then
		if player:getPosition() ~= Position(33854, 31853, 6) then
			return true
		end
-- Poss de los players			
	for x = 33854, 33858 do
	local playerTile = Tile(Position(x, 31853, 6)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GoshnarsCruelty) > os.time() then
...
-- poss de los players
	for x = 33854, 33858 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GoshnarsCruelty, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
