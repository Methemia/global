...
local config = {
	bossName = "Demon",
	lockStorage = 50815111, -- globalstorage
...
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 30995, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.Goshnars) > os.time() then
...
-- poss de los players
	for x = 34062, 34066 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.Goshnars, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
