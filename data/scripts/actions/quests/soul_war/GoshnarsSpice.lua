local Goshnars = Action()
local config = {
	bossName = "Goshnar's Spice",
	lockStorage = 55955711, -- globalstorage
	bossPos = Position(34059, 31062, 11),
	centerRoom = Position(34059, 31062, 11), -- Center Room  
	exitPosition = Position(34094, 31065, 11), -- Exit Position
	newPos = Position(34061, 31068, 11),
	range = 20,
...
function Goshnars.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38451 then
		if player:getPosition() ~= Position(34093, 31063, 11) then
			return true
		end
-- Poss de los players			
	for x = 34093, 34097 do
	local playerTile = Tile(Position(x, 31063, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GoshnarsSpice) > os.time() then
...
-- poss de los players
	for x = 34093, 34097 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GoshnarsSpice, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
