...
function Cruelty.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38453 then
...
-- Poss de los players			
	...
	local playerTile = Tile(Position(x, 30996, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.Cruelty) > os.time() then
...
-- poss de los players
	for x = 34147, 34151 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.Cruelty, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
