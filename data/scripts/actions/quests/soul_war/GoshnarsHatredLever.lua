local HatredLever = Action()
local config = {
	bossName = "Goshnar's Hatred",
	lockStorage = 50824113, -- globalstorage
	bossPos = Position(34126, 30993, 11),
	centerRoom = Position(34126, 30993, 11), -- Center Room  
	exitPosition = Position(34149, 30993, 11), -- Exit Position
	newPos = Position(34125, 30999, 11),
	range = 20,
...
function HatredLever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38450 then
		if player:getPosition() ~= Position(34147, 30996, 11) then
			return true
		end
-- Poss de los players			
	for x = 34147, 34151 do
	local playerTile = Tile(Position(x, 30996, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GoshnarsHatred) > os.time() then
...
-- poss de los players
	for x = 34147, 34151 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GoshnarsHatred, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
...
HatredLever:aid(38450)
HatredLever:register()