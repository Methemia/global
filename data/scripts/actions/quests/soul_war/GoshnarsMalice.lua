...
local config = {
	...
	lockStorage = 50815211, -- globalstorage
	bossPos = Position(34092, 30991, 11),
	centerRoom = Position(34093, 30995, 11), -- Center Room  
	exitPosition = Position(34063, 30992, 11), -- Exit Position
	newPos = Position(34093, 31002, 11),
	range = 20,
...
function Goshnars.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 9826 and item.actionid == 38448 then
		if player:getPosition() ~= Position(34062, 30995, 11) then
			return true
		end
-- Poss de los players			
	for x = 34062, 34066 do
	local playerTile = Tile(Position(x, 30995, 11)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			if playerTile:getStorageValue(Storage.Tibiana.GoshnarsMalice) > os.time() then
...
-- poss de los players
	for x = 34062, 34066 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Tibiana.GoshnarsMalice, os.time() + 15 * 60 * 60) -- + 20 * 60 * 3600
