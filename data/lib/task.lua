...
task = {
    ...
    tasks = { -- You can add as many tasks as you want
        [1] = {
            name = "The Curse Spreads", -- Name of Task
            description = "some were-beast leaders have been found to be responsible for spreading the disease, and he needs your help killing them. You'll find them behind magical gates located in Edron's and Cormaya's dungeons. Please kill some of them for me.", -- What the NPC tells the player to do
            ...
            toKill = 5, -- How many monsters the player needs to kill
        ...
        if string.lower(target:getName()) == string.lower(task.tasks[currentTask].monsters[i]) then
            ...
            if task:getMonstersLeft(player) == 0 then
                if task:setCompleted(player, currentTask) then
                    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have completed your task.")
                    task:debug(string.format("%s has completed %s.", player:getName(), task.tasks[currentTask].name))
                    return true
                end
            end

            player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You killed a " .. string.lower(target:getName()) .. ". Only " .. task:getMonstersLeft(player) .. " left to kill!")
...
function task:reward(player, npcHandler)
    ...
    for i = 1, #task.tasks[t].rewards do
        ...
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("You received %dx %s(s).", task.tasks[t].rewards[i].count, task.tasks[t].rewards[i].name))
