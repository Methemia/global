if not Quests then
	Quests = {
		...
		[42] = {
			...
			missions = {
				[1] = {
					name = "Mision Final: A Dream Come True",
					storageId = Storage.ThreatenedDreams.FinalMission,
					missionId = 10387,
					startValue = 1,
					endValue = 5,
					states = {
						[1] =  function(player)local text = "Maelyrra te ha pedido que mates 200 Enfeebled Silencers y/o Weakened Frazzlemaw, llevas "..player:getStorageValue(Storage.ThreatenedDreams.MiniRoshaCounter).." /200.\nTambien debes matar a Kroazur, lo cual " if player:getStorageValue(Storage.ThreatenedDreams.KroazurKilled) == 1 then text = text .. "ya hiciste." else text = text .. "aun no haces." end return text end,
						[2] =  function(player)local text = "Maelyrra te pidio que liberes las hadas de Feyrist y recuperes el dark moon mirror.\nLlevas "..player:getStorageValue(Storage.ThreatenedDreams.FairyFreeCount).."/5 hadas liberadas, y " if player:getStorageValue(Storage.ThreatenedDreams.RewardFinal1) == 1 then text = text .. "ya recuperaste el dark moon mirror." else text = text .. "aun no recuperas el dark moon mirror." end return text end,
						[3] = "Maelyrra te pidio que cargues de poder los 3 objetos que tienes.\nEl dark moon mirror con el poder de la luna en la noche.\nEl empty starlight vial con el poder de las estrellas en la noche.\nEl dark sun catcher con el poder del sol en el dia.",
						[4] = function(player) return "Maelyrra te pidio que repares la barrera magica de Feyrist con los objetos cargados de luz.\nLlevas "..player:getStorageValue(Storage.ThreatenedDreams.StoneMosaicCounter).."/5 Stone Mosaic con el shining sun catcher.\nLlevas "..player:getStorageValue(Storage.ThreatenedDreams.DreamBirdTreeCounter).."/5 Dream Bird Tree con el gleaming starlight vial.\nLlevas "..player:getStorageValue(Storage.ThreatenedDreams.MoonSculptureCounter).."/5 Moon Sculpture con el lit moon mirror." end,
						[5] = "Devolviste a Feyrist su paz, aunque sea momentanea.\nLa blossom bag que Maelyrra te ha regalado te recordara tus esfuerzos en Feyrist.",
						}
				},
				[2] = {
		...
		[47] = {
			name = "Rascaccon Dreams",
			startStorageId = Storage.PiratsTails.Questline,
			startStorageValue = 1,
			missions = {
				[1] = {
					name = "Mision Final: A Dream Come True",
					storageId = Storage.PiratsTails.RatMiral.FinalMission,
					missionId = 90604,
					startValue = 1,
					endValue = 2,
					states = {
						[1] =  function(player)local text = "You need need you to go kill Tentuglys Head 1 times "..player:getStorageValue(Storage.PiratsTails.RatMiral.BossCounter).." /1.\n " if player:getStorageValue(Storage.PiratsTails.RatMiral.BossCounter) == 1 then text = text .. "ya hiciste." else text = text .. "aun no haces." end return text end,
						[2] =  function(player)local text = "You need need you to go kill Ratmiral 2 times "..player:getStorageValue(Storage.PiratsTails.RatMiral.ClickCounter).." /1.\n " if player:getStorageValue(Storage.PiratsTails.RatMiral.BossCounter) == 1 then text = text .. "ya hiciste." else text = text .. "aun no haces." end return text end,
						}
				},
				[2] = {
					name = "Mission 02: Tattered Swan Feathers",
