...
class Player final : public Creature, public Cylinder
{
	public:
		...
		uint32_t getLevel() const {
			if (activeTempLevel) return tempLevel;
			return level;
		}
		...
		int32_t getMaxHealth() const final {
			if (activeTempLevel) return std::max<int32_t>(1, tempHealthMax + varStats[STAT_MAXHITPOINTS]);
			return std::max<int32_t>(1, healthMax + varStats[STAT_MAXHITPOINTS]);
		}
		uint32_t getMaxMana() const {
			if (activeTempLevel) return std::max<int32_t>(0, tempManaMax + varStats[STAT_MAXMANAPOINTS]);
 ...
 private:
    ...
    uint64_t asyncOngoingTasks = 0;
		...
		bool activeTempLevel = false;
		uint32_t tempLevel = 1;
		int32_t tempHealthMax = 1;
		uint32_t tempManaMax = 0;
		...
		uint32_t transferableCoinBalance = 0;
