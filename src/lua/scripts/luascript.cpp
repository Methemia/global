...
void LuaScriptInterface::registerFunctions()
{
  ...
  registerEnum(WEAPON_QUIVER)
	...
	registerEnumIn("configKeys", ConfigManager::PUSH_WHEN_ATTACKING)
	...
	registerMethod("Player", "setTempLevel", LuaScriptInterface::luaPlayerSetTempLevel);
...
int LuaScriptInterface::luaPlayerSetTempLevel(lua_State* L)
{
	// player:setTempLevel(false)
	// player:setTempLevel(level)
	Player* player = getPlayer(L, 1);
	if (player) {
		if (isBoolean(L, 2)) {
			bool active = getBoolean(L, 2);
			player->activeTempLevel = active;
			player->health = std::min<int32_t>(player->health, player->getMaxHealth());
			player->mana = std::min<int32_t>(player->mana, player->getMaxMana());
			g_game.addPlayerMana(player);
			player->updateBaseSpeed();
			player->setBaseSpeed(player->getBaseSpeed());
			...
			player->tempLevel = getNumber<int32_t>(L, 2);
			player->tempHealthMax = 185 + ((player->tempLevel - 8) * player->getVocation()->getHPGain());
			player->tempManaMax = 90 + ((player->tempLevel - 8) * player->getVocation()->getManaGain());
			player->health = std::min<int32_t>(player->health, player->tempHealthMax);
			player->mana = std::min<int32_t>(player->mana, player->tempManaMax);
			g_game.addPlayerMana(player);
			player->activeTempLevel = true;
			player->updateBaseSpeed();
			player->setBaseSpeed(player->getBaseSpeed());
			g_game.changeSpeed(player, 0);
			g_game.addCreatureHealth(player);
